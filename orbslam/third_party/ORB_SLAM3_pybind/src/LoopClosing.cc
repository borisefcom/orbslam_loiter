...
void LoopClosing::CorrectLoop()
{
    ...
    // Expose a rigid correction between the pre/post loop-closure "world" frames,
    // derived from the change in the current keyframe pose. This provides a cheap
    // initial alignment for external voxel submaps.
    //
    // g2oScw and mg2oLoopScw are both (world -> camera) Sim3 for the *same* camera
    // frame, so:
    //    T_w_new_w_old = (Scw_new)^-1 * Scw_old
    // maps points from the old world frame into the new world frame.
    {
        AlignmentEvent ev;
        ev.type = 0; // loop closure
        ev.t_s = mpCurrentKF ? mpCurrentKF->mTimeStamp : 0.0;
        Map* pMapA = mpCurrentKF ? mpCurrentKF->GetMap() : static_cast<Map*>(NULL);
        Map* pMapB = mpLoopMatchedKF ? mpLoopMatchedKF->GetMap() : static_cast<Map*>(NULL);
        ev.map_a = pMapA ? static_cast<std::uint64_t>(pMapA->GetId()) : 0ULL;
        ev.map_b = pMapB ? static_cast<std::uint64_t>(pMapB->GetId()) : 0ULL;
        ev.kf_a = mpCurrentKF ? static_cast<std::uint64_t>(mpCurrentKF->mnId) : 0ULL;
        ev.kf_b = mpLoopMatchedKF ? static_cast<std::uint64_t>(mpLoopMatchedKF->mnId) : 0ULL;

        const g2o::Sim3 g2oT_new_old = mg2oLoopScw.inverse() * g2oScw;
        ev.scale = static_cast<double>(g2oT_new_old.scale());
        const Eigen::Matrix3d R = g2oT_new_old.rotation().toRotationMatrix();
        const Eigen::Vector3d t = g2oT_new_old.translation();
        ev.T_new_old = {
        ...
        if(mpSystem)
            mpSystem->PushAlignmentEvent(ev);
    }
...
void LoopClosing::MergeLocal2()
{
...

    {
        ...
        // Expose a rigid map-level correction (Sim3) so external voxel submaps can be
        // aligned without running full ICP. The event reports T_new_old where:
        //   p_new = s * R * p_old + t
        // with (R,t,s) taken from this event.
        {
            AlignmentEvent ev;
            ev.type = 1; // merge
            ev.t_s = mpCurrentKF ? mpCurrentKF->mTimeStamp : 0.0;
            ev.map_a = pCurrentMap ? static_cast<std::uint64_t>(pCurrentMap->GetId()) : 0ULL;
            ev.map_b = pMergeMap ? static_cast<std::uint64_t>(pMergeMap->GetId()) : 0ULL;
            ev.kf_a = mpCurrentKF ? static_cast<std::uint64_t>(mpCurrentKF->mnId) : 0ULL;
            ev.kf_b = mpMergeMatchedKF ? static_cast<std::uint64_t>(mpMergeMatchedKF->mnId) : 0ULL;
            ev.scale = static_cast<double>(mSold_new.scale());

            const Eigen::Matrix3d R = mSold_new.rotation().toRotationMatrix();
            const Eigen::Vector3d t = mSold_new.translation();
            ev.T_new_old = {
                R(0,0), R(0,1), R(0,2), t(0),
                R(1,0), R(1,1), R(1,2), t(1),
                R(2,0), R(2,1), R(2,2), t(2),
