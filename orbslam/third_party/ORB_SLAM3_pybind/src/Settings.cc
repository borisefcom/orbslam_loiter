...
namespace ORB_SLAM3 {
    ...
    void Settings::readCamera2(cv::FileStorage &fSettings) {
        ...
        if (cameraType_ == PinHole) {
            ...
            // Rectified stereo assumes identical ideal pinhole intrinsics for both cameras.
            calibration2_ = new Pinhole(*static_cast<Pinhole*>(calibration1_));
            originalCalib2_ = new Pinhole(*static_cast<Pinhole*>(originalCalib1_));
        }
    ...
    void Settings::readIMU(cv::FileStorage &fSettings) {
        ...
        bool foundFastInit = false;
        const int fastInit = readParameter<int>(fSettings, "IMU.fastInit", foundFastInit, false);
        imuFastInit_ = foundFastInit && (fastInit != 0);
    }
