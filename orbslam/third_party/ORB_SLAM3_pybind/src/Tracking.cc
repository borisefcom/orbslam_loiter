...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    mFastInit = settings->imuFastInit();
    if(mFastInit)
...
void Tracking::PreintegrateIMU()
{
    ...
    // Always allocate the per-frame preintegration container. Even if we end up with 0 usable
    // IMU deltas (n<=0), downstream code (PredictStateIMU) assumes this pointer is valid and
    // will dereference it unconditionally. Returning early here leaves it null and can crash.
    IMU::Preintegrated* pImuPreintegratedFromLastFrame = new IMU::Preintegrated(mLastFrame.mImuBias,mCurrentFrame.mImuCalib);

    // NOTE (stability / pybind integrations):
    // On some real-time pipelines we can hit tracking resets during IMU initialization that leave
    // `mpImuPreintegratedFromLastKF` unset. The original code prints a warning but still
    // dereferences the null pointer, which results in a hard crash (access violation).
    //
    // A missing "from last KF" preintegration should simply prevent IMU prediction for this frame.
    // Allocate a fresh preintegration container so we can continue safely.
    if (!mpImuPreintegratedFromLastKF)
    {
        cout << "mpImuPreintegratedFromLastKF does not exist (allocating)" << endl;
        if(!mpImuCalib)
        {
            delete pImuPreintegratedFromLastFrame;
            return;
        }
        mpImuPreintegratedFromLastKF = new IMU::Preintegrated(IMU::Bias(),*mpImuCalib);
    }
...
void Tracking::Track()
{
    ...
    if(mState!=NO_IMAGES_YET && mLastFrame.isSet())
    {
...
void Tracking::ResetActiveMap(bool bLocMap)
{
    ...
    if(mbOnlyTracking)
    {
        // Localization-only: keep the existing map and just reset tracking state.
        ResetTrackingOnly();
        return;
    }

    // Stability: when resetting the active map from a live RGB-D-inertial stream, ensure we drop any
    // accumulated IMU samples from the previous map segment. Otherwise the next frame can preintegrate
    // across a reset boundary, which can destabilize inertial initialization and has been observed to
    // trigger hard crashes on some real-time pipelines.
    {
        unique_lock<mutex> lock(mMutexImuQueue);
        mlQueueImuData.clear();
    }
...
void Tracking::ResetTrackingOnly()
{
    unique_lock<mutex> lock(mMutexImuQueue);
    mlQueueImuData.clear();
    mbVelocity = false;
...
void Tracking::UpdateFrameIMU(const float s, const IMU::Bias &b, KeyFrame* pCurrentKeyFrame)
{
    ...
    {
        // During resets the tracking thread may overwrite mCurrentFrame. Avoid
        // deadlocks and crashes by bailing out when the state is no longer OK.
        if(mState == NO_IMAGES_YET || mState == LOST)
            return;
        if(++wait_iters > 20000) // ~10s max (usleep is microseconds)
            return;
        usleep(500);
    }
