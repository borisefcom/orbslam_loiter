...
struct VocabularyCache {
    std::mutex mutex;
    std::map<std::string, ORBVocabulary*> vocab_by_path;
};

VocabularyCache& GetVocabularyCache()
{
    static VocabularyCache cache;
    return cache;
}

ORBVocabulary* LoadVocabularyCached(const std::string& vocab_path)
{
    VocabularyCache& cache = GetVocabularyCache();
    std::lock_guard<std::mutex> lock(cache.mutex);
    auto it = cache.vocab_by_path.find(vocab_path);
    if (it != cache.vocab_by_path.end())
    {
        cout << "Using cached ORB Vocabulary." << endl << endl;
        return it->second;
    }

    cout << endl << "Loading ORB Vocabulary. This could take a while..." << endl;
    ORBVocabulary* vocabulary = new ORBVocabulary();
    bool bVocLoad = vocabulary->loadFromTextFile(vocab_path);
    if(!bVocLoad)
    {
        cerr << "Wrong path to vocabulary. " << endl;
        cerr << "Falied to open at: " << vocab_path << endl;
        exit(-1);
    }
    cache.vocab_by_path.emplace(vocab_path, vocabulary);
    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    {
        //Load ORB Vocabulary
        mpVocabulary = LoadVocabularyCached(strVocFile);
...
bool System::SwitchSensor(const string &strSettingsFile, const eSensor sensor, const bool localizationOnly)
{
    if(mbShutDown)
    {
        cerr << "System is shutdown, cannot switch sensor." << endl;
        return false;
    }

    const bool target_inertial = (sensor == IMU_STEREO || sensor == IMU_MONOCULAR || sensor == IMU_RGBD);
    if(mpAtlas && mpAtlas->isInertial())
    {
        if(!target_inertial)
        {
            if(!localizationOnly)
            {
                cerr << "[System] Atlas is inertial; non-IMU switch is only supported in localization-only mode." << endl;
                return false;
            }
            cout << "[System] Atlas is inertial; switching to non-IMU in localization-only mode." << endl;
        }
    }
    ...
    bool loc_only = localizationOnly;
    if(target_inertial && loc_only)
    {
        cout << "[System] IMU localization-only is not supported; enabling Local Mapping." << endl;
        loc_only = false;
    }
    ...
    Settings* newSettings = nullptr;
    cv::FileNode node = fsSettings["File.version"];
    if(!node.empty() && node.isString() && node.string() == "1.0")
    {
        newSettings = new Settings(strSettingsFile, sensor);
        mStrLoadAtlasFromFile = newSettings->atlasLoadFile();
        mStrSaveAtlasToFile = newSettings->atlasSaveFile();
        cout << (*newSettings) << endl;
    }

    if(settings_ && settings_ != newSettings)
    {
        mRetiredSettings.push_back(settings_);
    }
    ...
    {
        if(mpMapDrawer)
            mpMapDrawer->newParameterLoader(settings_);
        if(mpViewer)
            mpViewer->newParameterLoader(settings_);
    }

    Tracking* newTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                                        mpAtlas, mpKeyFrameDatabase, strSettingsFile,
                                        mSensor, settings_, string());
    newTracker->SetLocalMapper(mpLocalMapper);
    newTracker->SetLoopClosing(mpLoopCloser);
    if(mpViewer)
        newTracker->SetViewer(mpViewer);
    newTracker->InformOnlyTracking(loc_only);
    newTracker->mState = Tracking::LOST;
    newTracker->mLastProcessedState = Tracking::LOST;

    Tracking* oldTracker = mpTracker;
    mpTracker = newTracker;
    if(mpLocalMapper)
        mpLocalMapper->SetTracker(mpTracker);
    if(mpLoopCloser)
        mpLoopCloser->SetTracker(mpTracker);
    ...
    if(oldTracker)
        mRetiredTrackers.push_back(oldTracker);

    if(!loc_only && mpLocalMapper)
        mpLocalMapper->Release();

    return true;
}

bool System::IsAtlasInertial()
{
    if(!mpAtlas)
        return false;
    return mpAtlas->isInertial();
}
...
long unsigned int System::GetCurrentMapId()
{
    if(!mpAtlas)
        return 0;
    Map* pMap = mpAtlas->TryGetCurrentMap();
    if(!pMap)
        return 0;
    return pMap->GetId();
}
...
int System::GetCurrentMapChangeIndex()
{
    ...
    if(!pMap)
        return 0;
    return pMap->GetMapChangeIndex();
}

int System::GetCurrentMapBigChangeIndex()
{
    if(!mpAtlas)
        return 0;
    Map* pMap = mpAtlas->TryGetCurrentMap();
    if(!pMap)
        return 0;
    return pMap->GetLastBigChangeIdx();
}
...
std::vector<long unsigned int> System::GetAtlasMapIds()
{
    std::vector<long unsigned int> out;
    if(!mpAtlas)
        return out;
    ...
    for(Map* pMap : maps)
    {
        if(!pMap)
            continue;
        out.push_back(pMap->GetId());
    }
    return out;
}

std::vector<AlignmentEvent> System::PopAlignmentEvents()
{
    std::vector<AlignmentEvent> out;
    unique_lock<mutex> lock(mMutexAlignmentEvents);
    if(mAlignmentEvents.empty())
        return out;
    out.reserve(mAlignmentEvents.size());
    while(!mAlignmentEvents.empty())
    {
        out.push_back(mAlignmentEvents.front());
        mAlignmentEvents.pop_front();
    }
    return out;
}

void System::PushAlignmentEvent(const AlignmentEvent &ev)
{
    unique_lock<mutex> lock(mMutexAlignmentEvents);
    mAlignmentEvents.push_back(ev);
}

void System::DebugPushAlignmentEvent(const AlignmentEvent &ev)
{
    PushAlignmentEvent(ev);
}
...
void System::Shutdown()
{
    ...
    for(Tracking* tracker : mRetiredTrackers)
    {
        delete tracker;
    }
    mRetiredTrackers.clear();
    for(Settings* settings : mRetiredSettings)
    {
        delete settings;
    }
    mRetiredSettings.clear();
...
{
    vector<MapPoint*> tracked;
    {
        unique_lock<mutex> lock(mMutexState);
        tracked = mTrackedMapPoints;
    }
    ...
    const vector<MapPoint*> mapPoints = pMap->GetAllMapPoints();
    std::unordered_set<MapPoint*> valid(mapPoints.begin(), mapPoints.end());

    vector<MapPoint*> filtered;
    filtered.reserve(tracked.size());
    for(MapPoint* p : tracked)
    {
        if(!p)
            continue;
